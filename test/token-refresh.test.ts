...
import { jsonResponse, makeAccessToken, makeAuth, makeJwt, mockFetch } from './helpers.js';
...
describe('token refresh', () => {
	it('posts refresh_token grant and returns normalized credentials', async () => {
		...
		const fetchImpl = mockFetch(async (url) => {
			if (String(url) === 'https://auth.example.test/.well-known/openid-configuration') {
				return jsonResponse({ token_endpoint: 'https://auth.example.test/oauth/token' });
			}
		...
		expect(String(url)).toBe('https://auth.example.test/oauth/token');
		expect(String(init?.body)).toContain('grant_type=refresh_token');
		expect(String(init?.body)).toContain('client_id=client-test');
	});

	it('infers the OAuth client id from the ID token audience before any access token audience', async () => {
		const access = makeAccessToken('acct-refresh');
	...
	it('fails when refresh response does not include a token account claim', async () => {
		...
		const fetchImpl = mockFetch(async (url) => {
			...
			return jsonResponse({ access_token: badAccess, refresh_token: 'new-refresh', expires_in: 3600 });
		});
	...
	it('does not expose non-OK response bodies in refresh errors', async () => {
		...
		const fetchImpl = mockFetch(async (url) => {
			...
			return new Response('refresh_token=secret-refresh', { status: 400, statusText: 'Bad Request' });
		});
	...
	it('fails when refresh response is missing token fields', async () => {
		...
		const fetchImpl = mockFetch(async (url) => {
			...
			return jsonResponse({ access_token: makeAccessToken('acct-refresh') });
		});
	...
	it('uses an explicit token URL without OAuth discovery', async () => {
		...
		const fetchImpl = mockFetch(async () => jsonResponse({ access_token: access, refresh_token: 'new-refresh' }));
